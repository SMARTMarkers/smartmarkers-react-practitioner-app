...
export const FhirProvider: React.FC<FhirProviderProps> = props => {
    ...
    const { iss, client_id, redirectUri, clientSecret, scope, promisSettings } = props
    const [isAuthenticated, setIsAuthenticated] = React.useState(false)
    const [user, setUser] = React.useState<User | null>(null)
    const [fhirClient, setFhirClient] = React.useState<Client | null>(null)
    const [fhirPromisClient, setFhirPromisClient] = React.useState<Client | undefined>(
    ...
    const loginCallback = async () => {
        await FHIR.oauth2
            ...
            .then(user => {
                const item = user.name[0]
                const name = [item.prefix.join(' '), item.given.join(' '), item.family].join(' ')
                const u: User = {
                    id: user.id ? user.id : '',
                    name,
                    gender: user.gender,
