...
export class ServiceRequest implements IServiceRequest {
    ...
    public getTitle() {
        ...
        if (this.code && this.code.coding && this.code.coding[0] && this.code.coding[0].display)
            return this.code.coding[0].display
        if (this.category && this.category[0] && this.category[0].text) return this.category[0].text
    ...
    async getInstrument() {
        ...
        if (reference) {
            const response =
                reference.toLocaleLowerCase().startsWith('https://mss.fsm.northwestern.edu') &&
                this.promisServer
                    ? await this.promisServer.getPromisResource<IQuestionnaire>(reference)
                    : await this.server.getInstrumentByReference<IQuestionnaire>(reference)

            if (response) {
                const instrumentFactory = new InstrumentFactory(this.server, this.promisServer)
